...
document.getElementById('launch_button').addEventListener('click', async e => {
    ...
    try {
        ...
        showLaunchFailure('啟動期間發生錯誤', '請洽社群管理員以獲取更多詳細信息')
    }
})
    ...
    if(jvmDetails == null) {
        ...
        setOverlayHandler(() => {
            ...
            try {
                ...
                showLaunchFailure('Java 下載期間發生錯誤', '請洽詢社群管理員以獲取更多詳細信息')
            }
        })
        setDismissHandler(() => {
...
async function dlAsync(login = true) {
    ...
    try {
        ...
        showLaunchFailure('致命錯誤', '無法載入伺服器索引的內容 請洽詢社群管理員以獲取更多詳細信息')
        return
    }
    ...
    fullRepairModule.childProcess.on('error', (err) => {
        ...
        showLaunchFailure('啟動時發生錯誤', err.message || '請洽詢社群管理員以獲取更多詳細信息')
    })
    fullRepairModule.childProcess.on('close', (code, _signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`Full Repair Module 以代碼 ${code} 結束, 發生錯誤`)
            showLaunchFailure('啟動時發生錯誤', '請洽詢社群管理員以獲取更多詳細信息')
        }
    })
    ...
    try {
        ...
        showLaunchFailure('檔案驗證時發生錯誤', err.displayable || '請洽詢社群管理員以獲取更多詳細信息')
        return
    }
    

    if(invalidFileCount > 0) {
        loggerLaunchSuite.info('下載檔')
        ...
        try {
            ...
            showLaunchFailure('檔案下載時發生錯誤', err.displayable || '請洽詢社群管理員以獲取更多詳細信息')
            return
        }
    } else {
